...
### 1.1 Monty Hall Monte Carlo

Monte Carlo simulation is a flexible way to estimate the probability distribution of the outcome from a probabilistic (uncertain) event with a known (or assumed) structure. This is done by generating many manifestations of the probabilistic event and then analyzing the proportion of times each possible outcome appeared.
...
-   Monty will always eliminate an option by opening a door that is not your choice and contains a goat. After Monty eliminates one option, the car must be in one of two places: behiind our selected door or behind the door we can switch to.

-   If our initial choice was correct, we will always win if we stay with our current choice (and lose if we switch)

-   If our initial choice was incorrect, we will always win if we switch doors (and lose if we stay with our current choice)
...
#.  your choice or the door with the car (could be the same door). You can start with a 
#.  vector with all three possibilities (1, 2, and 3), but then remove the 
#.  option(s) corresponding with your choice and the prize.
monty_options <- NA

# Randomly select one of the doors from Monty's options for him to reveal
monty_reveal <- NA

# Assign the variable optimal_choice with 'stay' if you wold win by staying with your 
#.  choice and with 'switch' if you would win by switching. 
...
If we run this simulation many times, the proportion of replications runs in which the player would win by switching doors will converge to the probability switching is better. In this code chunk, replicate the simulation 100,000 times.
...
```{r}
#Create a vector of the simulation sizes we want to analyze (No need to edit)
...
This next code chunk should generate the plot without any edits from you, as long as the preceding chunks are correct. But as always, take a look to try and understand what it's doing. I added comments to explain the code.
...
ggplot(data = df_sim_size_outcomes,#all ggplots use dataframes
       #x and y values are defined in aes() because they are considered "aesthetics"
...
If done correctly, your plot should show that our Monte-Carlo estimate is 'noisy' for small sample sizes but converges to the expected output as the number replications increases.

### 1.2 Monty Hall decision tree
...
#The probability your initially selected door contains a car or a goat. Probabilities must sum to 1.
p_car_initial = NA
...
Hopefully this exercise has helped convince you that the Monty Hall solution, though counter-intuitive, is indeed correct. There are many ways to solve or explain this solution. If interested, you can check out the Wikipedia page for others.

## Section 2: A cohort model

In section 2, you will model the health of a cohort of 10,000 individuals over 30 years. Your model will have three health states: Healthy, Sick, and Dead. Individuals will all start healthy, but as time progresses some will get sick and some will die. You will generate a **"cohort trace plot"** that shows the percent of individuals in each health state at the end of each year (or each "timestep"). You will perform a simple scenario analysis and use the model to conduct a simplified health policy analysis, assessing the impact of a hypothetical vaccine.
...
We're going to develop a Markov model, a simple and commonly-used type of cohort state transition model. You'll learn a lot more about these later. For now, know that at the end of each timestep, individuals can transition from one state to another. An individual's probability of transitioning depends only on which state they are in currently. It doesn't matter how long they have been in that state or how many timesteps have passed. We're going to use a for loop to develop the Markov model, but you'll learn other ways to develop Markov models later in the course.

First, we create the matrix where we track our cohort's transition between the health states over time. This is sometimes called a cohort trace matrix. We can also fill in the first row of this matrix (timestep 0) with the initial distribution of the cohort using our parameter table.
...
One property of Markov model is that the number of people is conserved at each timestep; no one 'escapes' or enters the model as it runs. Therefore, summing across any row of the cohort trace matrix should give you the same value; the original cohort size (if not, you have an error). Try it out.
...
### 2.2 Vaccine policy analysis

Now that our model works, we want to put it into a function to make it easier to re-run with different parameter sets. Copy and paste code your code from section 2.1 into the function below that it takes a named list of parameters as inputs and returns the dataframe with the cohort trace.
...
#  probability of healthy to sick is decreased to 1%. (I did this one for you)
l_prms_vaccine <- l_prms #Copy the parameters
l_prms_vaccine$p_H_to_S <- 0.01 #Changed one value

#DEADLIER VARIANT SCENARIO
#  probability of sick to dead is increased to 12%
l_prms_deadlier <- NA
#IMPLEMENT THE CHANGE

#DEADLIER VARIANT AND VACCINE SCENARIO
...
# Do the same for the three new scenarios and use rbind() to append each of the new scenarios to the df_scenarios dataframe.

#  Original strain with vaccine

#  Variant without vaccine
...
Look at the output. Do the changes in how many individuals are in which state over time under each scenario make sense to you?
...
#.  replace the NAs with code to compute the quantities
total_deaths <- list(
...
By completing this assignment you've already gotten exposed to lots of important concepts from the class (decision trees, simulation, Markov cohort models, policy analysis). We'll learn a lot more about them throughout the semester.
