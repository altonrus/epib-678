...
This section is based on the model of a hypothetical sexually transmitted disease (loosely based on HIV) from [Menzies et. al. 2017](https://doi.org/10.1007/s40273-017-0494-4), a tutorial on Bayesian model calibration. They provide [R code for their implementation of model](https://doi.org/10.7910/DVN/QXM09F). The model used for this worked example was based on approaches used to modeling HIV in high-burden settings (5).
...
\frac{dL}{dt} &= cE - (\mu^{B} + \mu^{L}) L - r^L L & \text{late disease} \\
...
The force of infection $\lambda_t$ is defined as $\lambda_t = \rho \frac{E+L}{S+E+L+T}$, where $\rho$ is the effective contact rate. The model simulation begins at time 0 (corresponding to 1990) until time 51 (corresponding to 2040). Years 0-30 (1990-2020) are used to simulate the epidemic up until the present day, and years 30-51 (2020-2040) are used to project future outcomes for a policy comparison. The policy comparison is between two scenarios:

-   **Base case:** only individuals with late-stage disease can innitiate treatment
-   **Expanded treatment:** individuals with early-stage disease can also access treatment beginning in year 30.
...
Menzies et. al. implementation (without using DeSolve)
...
trace_model <- mod(par_vector)

trace_base_case <- trace_model$base_case
trace_expanded_tx <- trace_model$expanded_tx
...
```{r}
# Test out the deq_system_base function by running it with the base-case parameters and plotting the results

base_scenario <- ode(y = c(N=NA, S=NA, E=NA, L=NA, T=NA, D=NA), 
                 times = seq(0, 51, by=1/12), 
                 func = deq_system_base, 
...
#n_psa_iter <- 1000 #Uncomment this once your code is working
