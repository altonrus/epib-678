...
library(tidyverse)
library(dampack) #sensitivity analysis functions for CEA
...
## Recall: Decision-analytic model as function
...
-   **First order** **(stochastic):** variation between statistically identical patients
    ...
    -   [**Eliminate**]{.underline} (run enough simulations to make negligible)
...
prms <- list(cost_treat = 60000,      prob_death_noTreat = .3,
             rr_death_drug = 0.5,     QALY_survive = 12,
             cost_survive = 12000)

model <- function(prms){ # 5 parameters, 5 outcomes
  tot_cost_noTreat = (1-prms$prob_death_noTreat)*prms$cost_survive
  tot_cost_treat = (prms$cost_treat + 
        (1-prms$prob_death_noTreat*prms$rr_death_drug)*prms$cost_survive)
  tot_QALY_noTreat = (1-prms$prob_death_noTreat)*prms$QALY_survive
  tot_QALY_treat = (1-prms$prob_death_noTreat*prms$rr_death_drug)*prms$QALY_survive
  return(list(tot_cost_noTreat=tot_cost_noTreat,
              tot_cost_treat=tot_cost_treat,
              tot_QALY_noTreat=tot_QALY_noTreat,
              tot_QALY_treat=tot_QALY_treat,
...
```{r}
#| echo: true
l_basecase_results <- model(prms); l_basecase_results
```
...
-   Define the **minimum** and **maximum** plausible value for an uncertain parameter
    ...
    -   If study populations may systematically differ from modelled population, wider range may be warranted.
...
```{r}
#| echo: true
rrDrug_min = 0.2; rrDrug_max = 0.6; prms_temp <- prms
prms_temp[["rr_death_drug"]] <- rrDrug_min #Min value
result_rrDrug_min = model(prms_temp)
prms_temp[["rr_death_drug"]] <- rrDrug_max #Max value
result_rrDrug_max = model(prms_temp)
```
...
-   If you're certain it's monotonic, can just run the max, min, and basecase values

-   If any doubt, can check several points and plot
...
t_rr_drug_owsa <- data.frame(
  rr_death_drug = seq(from=rrDrug_min, to=rrDrug_max, length.out=20),
  ICER_treat = 0)
for (row in 1:nrow(t_rr_drug_owsa)){
  prms_temp <- prms
  prms_temp[["rr_death_drug"]] <- t_rr_drug_owsa[row, "rr_death_drug"]
  t_rr_drug_owsa[row, "ICER_treat"] <- model(prms_temp)$ICER_treat}
...
Assign a min and max plausible value to all uncertain parameters

```{r}
#| echo: true
t_owsa <- data.frame(
  name = names(prms),
  value = unname(unlist(prms)),
...
For each parameters, calculate outcome at min and max value

```{r}
#| echo: true
for (row in 1:nrow(t_owsa)){
  prms_temp <- prms #Reset to base case value (IMPORTANT!!!!)
  # Calculate ICER with parameter at min value
  prms_temp[[t_owsa[row,"name"]]] <- t_owsa[row,"value.min"]
  t_owsa[row, "ICER.min"] <- model(prms_temp)$ICER_treat
  # Calculate ICER with parameter at max value
  prms_temp[[t_owsa[row,"name"]]] <- t_owsa[row,"value.max"]
  t_owsa[row, "ICER.max"] <- model(prms_temp)$ICER_treat
}
...
p_tornado <- ggplot(data = t_owsa) +
  geom_segment(aes(x = reorder(name, ICER.range), # largest to smallest ICER range
...
## Ex: Is pathogen inactivation in Ghana cost-saving?
...
[Russell et. al. 2019](https://doi.org/10.7326/M18-2238)
:::

::: {.column width="35%"}
-   Tornado-like diagram

-   [**X-axis:**]{.underline} changing parameter value ±2 standard deviations from base case

-   Red text indicates parameter value at which preferred strategy changes.
:::
:::::

## Two way sensitivity analysis (TWSA)

Change a [**pair**]{.underline} of parameters over plausible ranges
...
t_twoway$ICER <- 0 # Add column for the ICER
...
for (row in 1:nrow(t_twoway)){
  prms_temp[["rr_death_drug"]] <- t_twoway[row,"rr_death_drug"]
  prms_temp[["prob_death_noTreat"]] <- t_twoway[row,"prob_death_noTreat"]
  t_twoway[row,"ICER"] <- model(prms_temp)$ICER_treat}
head(t_twoway)
```
...
::::: columns
::: {.column width="45%"}
![](figs/blood_safety_portfoloi_WTP_scenarios.png){width="433"}
:::

::: {.column width="55%"}
![](figs/outcomes_by_prevalence.png){width="473"}
:::
:::::

[Russell et. al. 2021](https://doi.org/10.1007/s10729-021-09557-1)

## 3-way sensitivity analysis example

::::: columns
::: {.column width="70%"}
![](figs/three_way_sens_analysis_TB_mHealth.png){width="678"}
:::

::: {.column width="30%"}
[Cost analysis of mobile health TB contact investigation. Turimumahoro et. al. 2022](https://doi.org/10.1371/journal.pone.0265033)
:::
:::::
...
**Want to know:** how robust are our results to [**all**]{.underline} the uncertainty in [**all**]{.underline} parameters?
...
-   Make probabilistic statements:
    ...
    -   *Treatment was cost-effective in 87% of PSA iterations*
...
[**Remember**]{.underline}: We usually want experts' estimate of uncertainty in parameter mean, not individual-level variance

## Multinomial probabilities

-   Probability of each branches from decision tree node (or transitions from Markov state) must sum to 1

-   [**2 outcomes:**]{.underline} no problem. Sample P(A), set P(B) = 1 - P(A)
...
## Dirichlet: beta for multinomial probabilities
...
## Where can Dirichlet be used for PSA?
...
The [**expected value (mean)**]{.underline} of a PSA distribution should be the base case value. For asymmetric distributions, median will not equal the mean.

-   To simplify this in-class example, I set the base case value as the median.

-   Assignment 4 will include functions to fit a distribution to the mean and two quantiles (e.g., 0.025 and 0.975 percentile)
...
prm_temp <- as.list(t_psa[row,])
psa_output <- data.frame(model(prm_temp))
for (row in 2:nrow(t_psa)){
  prm_temp <- as.list(t_psa[row,])
  psa_output <- rbind(psa_output, data.frame(model(prm_temp)))
}
...
-   [X-axis:]{.underline} willingness-to-pay thresholds

-   [Y-axis:]{.underline} estimated probability intervention preferred (most cost-effective) given WTP

-   **Frontier**: intervention with the greatest expected NMB

    -   Optimal strategy is one that maximizes expected NMB
    -   Sometimes differs from strategy with the greatest probability of being preferred

## More on frontier

Strategy with the greatest probability of cost-effectiveness may not have the largest expected NMB.
...
Expected net monitary benefit foregone because you choose a less-than-optimal intervention
...
-   **Design parameters** like discount rate, time horizon should not

    -   Can analyze in scenario analysis
...
-   In practice, analysts often treat PSA inputs as independent
